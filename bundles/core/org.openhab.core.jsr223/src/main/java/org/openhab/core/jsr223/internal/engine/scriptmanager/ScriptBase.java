...
public class ScriptBase {
    ...
    public ScriptBase(ScriptBase sb) {
        ...
        scriptManager = sb.getScriptManager();
        file = sb.getFile();
        loaded = sb.isLoaded();
    }
     ...
     *
     * later we will get Autoimports for Classes in Nashorn: further information:
     ...
     * http://mail.openjdk.java.net/pipermail/nashorn-dev/2015-February/004177.html Using Nashorn with interfaces loaded
     * from custom classloaders, "script function" as a Java lambda:
     *
     * engine.put("JavaClass", (Function<String, Class>) s -> { try { // replace this whatever Class finding logic here
     * // say, using your own class loader(s) based search Class<?> c = Class.forName(s); logger.error("Class " +
     * c.getName()); logger.error("s " + s); return Class.forName(s); } catch (ClassNotFoundException cnfe) { throw new
     * RuntimeException(cnfe); } }); engine.eval("var System = JavaClass('java.lang.System').static");
